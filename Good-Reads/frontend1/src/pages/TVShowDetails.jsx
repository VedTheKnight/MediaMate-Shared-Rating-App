...
function TVShowDetails() {
  ...
  const fetchBookDetails = useCallback(async (bookId) => {
    try {
      ...
      const response = await fetch(`${API_BASE}/content/tv/${bookId}`, { credentials: "include" });
  ...
  const addToWatchlist = async () => {
    try {
      ...
      if (existingItem) {
        ...
        alert("TV Show updated in watchlist!");
      } else {
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      await fetch(`${API_BASE}/content/tv/${id}/review`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: reviewText }),
  ...
  const fetchFilteredReviews = useCallback(async (filter) => {
    try {
      const response = await fetch(`${API_BASE}/content/tv/${id}/reviews?sentiment=${filter}`, { 
        credentials: "include" 
      });
