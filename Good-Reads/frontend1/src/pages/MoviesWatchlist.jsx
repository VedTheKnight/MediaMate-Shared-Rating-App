...
function MoviesWatchlist() {
  ...
  const fetchWatchlist = async () => {
    try {
      ...
      const booksData = data.filter((item) => item.content_type === "Movie");
      setBooks(booksData);
  ...
  const handleRating = async (bookId, value) => {
    try {
      await fetch(`${API_BASE}/content/Movie/${bookId}/rating`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  return (
    <Container style={styles.container}>
      <Typography variant="h4" gutterBottom style={styles.title}>Movies Watchlist</Typography>
