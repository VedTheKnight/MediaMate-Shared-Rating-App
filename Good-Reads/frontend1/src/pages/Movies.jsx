...
function Movies() {
  ...
  const fetchBooks = async () => {
    try {
      // const response = await fetch("http://localhost:4000/content/movie", { credentials: "include" });
      const response = await fetch(`${API_BASE}/content/movie`, { credentials: "include" });
  ...
  const handleRating = async (bookId, ratingValue) => {
    try {
      // const response = await fetch(`http://localhost:4000/content/movie/${bookId}/rating`, {
      //   method: "POST",
      //   credentials: "include",
      //   headers: { "Content-Type": "application/json" },
      //   body: JSON.stringify({ rating: ratingValue }),
      // });
      const response = await fetch(`${API_BASE}/content/movie/${bookId}/rating`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ rating: ratingValue }),
