...
# ========== CONFIG ==========
OMDB_API_KEY = '000000000'  # Replace with your real OMDb key
...
# Load existing genre mappings from previous runs (if any)
...
next_item_id = 1  # Simulated item IDs since we're not querying a DB
...
with open(INPUT_TSV, 'r', encoding='utf-8') as tsv_file:
    ...
    for row in reader:
        ...
        raw_genres = row['genres'].split(',') if row['genres'] != '\\N' else ['Unknown']
        ...
        content_inserts.append(
            ...
            f"VALUES ('{title}', '{description}', '{content_type}', '{release_date}', NULL, '{image_url}');"
            f" -- item_id = {next_item_id}"
        )

        # Insert genres and join rows
        for genre_name in raw_genres:
            ...
            join_inserts.append(
                f"INSERT INTO ContentItemGenre (item_id, genre_id) VALUES ({next_item_id}, {genre_id});"
            )
...
print(f"\u2705 Added {new_rows} new content items with genre links to {OUTPUT_SQL}")
